...
Für die Implementierung werden wir $\tilde{r}$ in Einheiten von $a$ und $\tilde{t}$ in Einheiten von $T/2\pi$. Es ergibt sich mit dem der Abschätzung des mittleren Radius von a und einer annähernd runden Kreisbahn für das Drehimpuls-Masse-Verhältnis die Näherung:
...
Dabei bietet uns das Runge-Kutter Verfahren numerische Stabilität (soweit das System selbst Stabile ist) und weist mit einem Verfahrensfehler von $\mathcal{O}(h^2)$ einen kleineren Fehler als das Eulerverfahren auf für kleinen Schrittweiten h.
...
## Kreisbahn
Für eine Exzentrizität von 0 erwarten wir eine gleichmäßige Eigenrotation, also ein Sinusverlauf.
```{r, echo=FALSE}
...
library(latex2exp)
plot(e0$theta%%(2*pi), e0$thetadot, "p", col = "dark blue",xlab=TeX(r'($\theta$)'), ylab = TeX(r'($T d_t \theta  / 2 \pi$)'))
plot(e0$theta, e0$thetadot, "p", col = "dark blue",xlab=TeX(r'($\theta$)'), ylab = TeX(r'($T d_t \theta  / 2 \pi$)'))
```
Wir finden in Abb. \ref{fig:grundEntwicklung2} ein sinusförmiges, regelmäßiges Verhalten der Eigenrotation. Bzw. bilden sich Bögen an den Rändern des Moduloplots. 

## Phobos
Wir verwenden nun die Werte von Phobos, also $\epsilon = 0,015$ und $\alpha = 0,83$. Wir erwarten, dass der varierende Abstand die Eigenrotation im allgemein Fall stören wird und wir somit recht chaotische Plots erhalten können.
...
plot(phobos1$theta%%(2*pi), phobos1$thetadot,"p", col = "dark blue",xlab=TeX(r'($\theta$)'), ylab = TeX(r'($T d_t \theta  / 2 \pi$)'), ylim=c(0, 1.5), sub=TeX(r'($\theta_0 = 0,\,\, \theta'_0 = 0,5,\,\, h=0,0001 $)'))
...
legend(2,1.3,legend=c(TeX(r'($\theta'_0 = 0.01$)'),TeX(r'($\theta'_0 = 0.1$)'),TeX(r'($\theta'_0 = 0.5$)'),TeX(r'($\theta'_0 = 1$)')), col = c("dark blue","dark red","red","pink"), lty=1:1)

plot(phobos1$theta, phobos1$thetadot,"p", col = "dark blue",xlab=TeX(r'($\theta$)'), ylab = TeX(r'($T d_t \theta  / 2 \pi$)'), ylim=c(0, 1.5), sub=TeX(r'($\theta_0 = 0,\,\, \theta'_0 = 0,5,\,\, h=0,0001 $)'))
...
In Abb. \ref{fig:grundEntwicklung3} können wir sowohl ein periodisches Verhalten, als auch ein chaotisches Verhalten sehen. Offenkundig haben die Startwerte einen deutlichen Einfluss auf das Verhalten des Mondes. Für die große Anfangsrotationengeschwindigkeiten erhalten wir die rötlich eingetragenen Ränder, welche im Plot ohne Modulo kaum Struktur tragen. Die sehr kleinen Startrotationen sind hingegen am unteren Rand der Grafik in Sinusformen aufzufinden. Wir habe in Abb \ref{fig:grundEntwicklung4} eine größere Reichweite an Anfangswerten dargestellt um ein Gefühl für die Verteilung zu gewinnen. Das grün gelbliche chaotische Verhalten stellt sich hier bei für $\theta$-Werte in der Größenordnung von 1/7 ein.

```{r, echo=FALSE, fig.cap="\\label{fig:grundEntwicklung4} Entwicklungskurven für $R_0 = 2.9$"}
library(latex2exp)
...
# Fazit
Wir erhalten bei spezifischen Werten periodisches Verhalten zwischen $\theta$ und $\dot \theta$. Dies ist im physikalischen Sinne auch nachvollziehbar da in diesen Fällen die Rotationsfrequenz der Eigenrotation über einen rationalen Faktor mit der der Rotation um Mars übereinstimmt.

Wir können aber sehen, dass dieses Verhalten nicht immer eintritt und wir im Allgemeinen ein chaotisches Verhalten vorfinden.
